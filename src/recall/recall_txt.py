...
class Txtrecall(object):
    def __init__(self, args, behavior_dataset=None, feature_dict=None, item_topk_click=None):
        ...
        self.txt_emb_df = pd.DataFrame()
        self.txt_emb_df["feat_vec"] = list(feature_dict.values())
        self.txt_emb_df['item_id'] = list(feature_dict.keys())
        self.txt_emb_df["feat_vec"] = self.txt_emb_df["feat_vec"].apply(lambda x: list(x[:128]))
        # Dictionary mapping of item index and item ID
        self.item_idx_2_rawid_dict = dict(zip(self.txt_emb_df.index, self.txt_emb_df['item_id']))
        self.item_rawid_2_idx_dict = dict(zip(self.txt_emb_df["item_id"], self.txt_emb_df.index))
        self.txt_emb_np = np.ascontiguousarray(list(self.txt_emb_df["feat_vec"].values), dtype=np.float32)
    ...
    def get_user_vec(self):
        ...
        for idx, user_id in enumerate(self.user_list):
            ...
            for loc, (i, click_time) in enumerate(user_hist_items):
                ...
                item_txt_vec = self.txt_emb_np[self.item_rawid_2_idx_dict[i]]
                if np.isnan(item_txt_vec)[0]:
                    vec.append([0] * 128)
                else:
                    vec.append(loc_weight * self.txt_emb_np[self.item_rawid_2_idx_dict[i]])
    ...
    def txt_based_recommend(self):

        item_index = faiss.IndexFlatIP(self.txt_emb_np.shape[1])
        item_index.add(self.txt_emb_np)
        ...
        for target_idx, sim_value_list, rele_idx_list in tqdm(zip(range(len(users_vec)), sim, idx)):
            ...
            if len(user_recall_items_dict[user_raw_id]) < self.args.recall_item_num:
                for i, item in enumerate(self.item_topk_click):
                    ...
                    if len(user_recall_items_dict[user_raw_id]) == self.args.recall_item_num:
                        break
            user_recall_items_dict[user_raw_id] = sorted(user_recall_items_dict[user_raw_id].items(), key=lambda x: x[1], reverse=True)[:self.args.recall_item_num]
