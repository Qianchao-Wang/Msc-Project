...
def get_content_similarity_feature(emd_dict, label_df, behavior_df):
    ...
    for index, data in tqdm(label_df.iterrows()):
        ...
        cos_sim = []
        for (item, click_time) in user_hist_items:
            if item in emd_dict.keys() and sim_item in emd_dict.keys():
                cos_sim.append(get_cos_sim(emd_dict[item], emd_dict[sim_item]))
            else:
                cos_sim.append(0)
    ...
    feature_df["max_content_sim"] = feature_df["max_content_sim"].apply(lambda x: 0.0 if np.isnan(x) else x)
    feature_df["mean_content_sim"] = feature_df["mean_content_sim"].apply(lambda x: 0.0 if np.isnan(x) else x)
    feature_df["var_content_sim"] = feature_df["var_content_sim"].apply(lambda x: 0.0 if np.isnan(x) else x)
...
def get_nearest_similarity_feature(i2i_sim_dict, w2v_emd_dict, txt_emd_dict, label_df, behavior_df, k=3):
        ...
        if len(user_hist_items) >= k:
            for (item, click_time) in user_hist_items[-k:]:
                ...
                if str(item) in w2v_emd_dict.keys() and str(sim_item) in w2v_emd_dict.keys():
                    ...
                    w2v_sim.append(0)
                if item in txt_emd_dict.keys() and sim_item in txt_emd_dict.keys():
                    txt_img_sim.append(get_cos_sim(txt_emd_dict[item], txt_emd_dict[sim_item]))
                else:
                    txt_img_sim.append(0)
    ...
    feature_df["content_sim_1"] = feature_df["content_sim_1"].apply(lambda x: 0.0 if np.isnan(x) else x)
    feature_df["content_sim_2"] = feature_df["content_sim_2"].apply(lambda x: 0.0 if np.isnan(x) else x)
    feature_df["content_sim_3"] = feature_df["content_sim_3"].apply(lambda x: 0.0 if np.isnan(x) else x)
...
if __name__ == "__main__":
    ...
    content_emd_dict = pickle.load(open(os.path.join(offline_sim_dir, 'item_content_vec_dict.pkl'), 'rb'))
    w2v_emd_dict = pickle.load(open(os.path.join(offline_sim_dir, 'item_w2v_emb.pkl'), 'rb'))
